    ...
    public class FileManagement
    {
        ...
        public void ShiftBytesFromFile(string InputFilePath, string TempFilePath, int Offset, out ErrorCode Code)
        {
            try
            {
                ...
                using (FileStream fsNewFile = new FileStream(TempFilePath, FileMode.CreateNew, FileAccess.Write))
                {
                    using (FileStream fsOldFile = new FileStream(InputFilePath, FileMode.Open, FileAccess.Read))
                    {
        ...
        public byte[] GetFileFragment(string InputFilePath, long Offset, long Count, out ErrorCode Code)
        {
            try
            {
                ...
                using (FileStream fsInput = new FileStream(InputFilePath, FileMode.Open)) // tworzę strumień czytający plik
                {
        ...
        public void SaveFileFragment(string SaveFilePath, long Offset, byte[] OutputFileFragment, out ErrorCode Code)
        {
            try
            {
                ...
                using (FileStream fsOutput = new FileStream(SaveFilePath, FileMode.Append)) // otwieram strumień zapisujący plik
                {
        ...
        private void SetFileReadable(string InputFilePath)
        {
            FileAttributes attributes = File.GetAttributes(InputFilePath);
