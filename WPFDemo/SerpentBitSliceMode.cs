    ...
    public class SerpentBitSliceMode : SerpentAlgorithm
    {
        ...
        public override object MakeKey(byte[] key)
        {
            ...
            for (i = 8, j = 0; i < 16; i++)
            {
                ...
                w[i] = (int)(t << 11 | (int)TripleRightShift(t, 21));
            }
            ...
            for (; i < limit; i++)
            {
                ...
                w[i] = (int)(t << 11 | TripleRightShift(t, 21));
            }
            ...
            for (i = 0; i < Rounds + 1; i++)
            {
                ...
                for (j = 0; j < 32; j++)
                {
                    z = sb[((TripleRightShift(x0, j)) & 0x01) |
        ...
        public override byte[] BlockEncrypt(byte[] InV, int inOffset, object sessionKey)
        {
            ...
            int x0 = (InV[inOffset++] & 0xFF) |
                     (InV[inOffset++] & 0xFF) << 8 |
                     (InV[inOffset++] & 0xFF) << 16 |
                     (InV[inOffset++] & 0xFF) << 24;
            int x1 = (InV[inOffset++] & 0xFF) |
                     (InV[inOffset++] & 0xFF) << 8 |
                     (InV[inOffset++] & 0xFF) << 16 |
                     (InV[inOffset++] & 0xFF) << 24;
            int x2 = (InV[inOffset++] & 0xFF) |
                     (InV[inOffset++] & 0xFF) << 8 |
                     (InV[inOffset++] & 0xFF) << 16 |
                     (InV[inOffset++] & 0xFF) << 24;
            int x3 = (InV[inOffset++] & 0xFF) |
                     (InV[inOffset++] & 0xFF) << 8 |
                     (InV[inOffset++] & 0xFF) << 16 |
                     (InV[inOffset++] & 0xFF) << 24;
            int y0, y1, y2, y3;
        ...
        private long TripleRightShift(long n, int s)
        {
            if (n >= 0)
                return n >> s;
            return (n >> s) + (2 << ~s);
        ...
        private int TripleRightShift(int n, int s)
        {
            if (n >= 0)
                return n >> s;
            return (n >> s) + (2 << ~s);
